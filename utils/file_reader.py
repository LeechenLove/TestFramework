...
class YamlReader:
    ...
    @property
    def data(self):
        #如果第一次调用data，读取yaml文档，否则返回之前保存的数据
        if not self._data:
    ...
    def data(self):
        if not self._data:
            ...
            if type(self.sheet) not in [int, str]:
                raise SheetTypeError('Please pass in type int and str, not{0}'.format(type(self.sheet)))
            ...
            if self.title_line:
                ...
                for col in range(1, s.nrows):
                    #依次遍历其他行，与首行组成dict，拼接至self._data
                    self._data.append(dict(zip(title, s.row_values(col))))
            else:
                for col in range(0, s.nrows):
                    #遍历所有行，拼到self._data中去
